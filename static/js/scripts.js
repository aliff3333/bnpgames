$(function () {
    ...
    if ($('#steps-slider').length) {
        ...
        slider.noUiSlider.on('update', function (values) {
            ...
            $('#encode4365gbf265g43d-range-from').text(numFormat(minPrice));
            $('#encode4365gbf265g43d-range-to').text(numFormat(maxPrice));

            // Update hidden input fields
            $('#id_min_price').val(minPrice);
            $('#id_max_price').val(maxPrice);
        });
    }
    /* encode4365gbf265g43d Range Slider */
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    scrollContainers.forEach(function (container) {
        ...
        container.addEventListener('mousemove', function (e) {
            ...
            const walk = (x - startX) * 0.75; // Adjust scrolling speed
...
document.addEventListener("DOMContentLoaded", function () {
    // Function to format price elements
    function formatPriceElements() {
        ...
        priceElements.forEach(function (element) {
            ...
            // Format the numeric value with Persian thousands separators
            const formattedPrice = numericValue.toLocaleString('fa-IR');
    ...
    // Format price elements when DOM content is loaded
    formatPriceElements();

    // Listen for htmx:afterSwap event to reformat price elements after HTMX requests
    document.addEventListener('htmx:afterSwap', function (event) {
        // Reformat price elements
        formatPriceElements();
    });
});